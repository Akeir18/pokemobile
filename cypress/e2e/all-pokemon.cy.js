...
context('all-pokemon', () => {
  ...
  it('can go with the menu', () => {
    cy.openMenu();
    cy.get('[data-cy="all-pokemon-link"]').click();
    cy.closeMenu();
    cy.url().should('eq', 'http://localhost:9000/pokemon');
  });

  it('displays all pokemon page on list as default', () => {
    cy.visit('/');
    cy.url().should('eq', 'http://localhost:9000/pokemon');
  });

  it('displays the data in list as default', () => {
  ...
  it('can change view type to grid', () => {
  ...
  it('can change view type to list', () => {
